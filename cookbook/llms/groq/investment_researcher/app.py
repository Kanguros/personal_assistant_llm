...
def main() -> None:
    ...
    if (
        "research_assistant" not in st.session_state
        or st.session_state["research_assistant"] is None
    ):
    ...
    if generate_report:
                ...
                with st.container():
                    ...
                    if company_info_full:
                        ...
                        company_info_cleaned = {
                            ...
                            "200 Day Average": company_info_full.get(
                            ...
                            "Operating Cash flow": company_info_full.get(
                                "operatingCashflow"
                            ),
                        ...
                        report_input += "---\n"
                status.update(
                    label="Company Info available", state="complete", expanded=False
                )
            ...
            with st.status("Getting Company News", expanded=True) as status:
                with st.container():
                    ...
                    if len(company_news) > 0:
                        ...
                        report_input += "---\n"
                status.update(
                    label="Company News available", state="complete", expanded=False
                )
            ...
            with st.status("Getting Analyst Recommendations", expanded=True) as status:
                with st.container():
                    ...
                    if not analyst_recommendations.empty:
                        ...
                        analyst_recommendations_md = (
                            analyst_recommendations.to_markdown()
                        )
                    ...
                    report_input += "---\n"
                status.update(
                    label="Analyst Recommendations available",
                    state="complete",
                    expanded=False,
                )
        ...
        if get_upgrades_downgrades:
            with st.status("Getting Upgrades/Downgrades", expanded=True) as status:
                with st.container():
                    ...
                    report_input += "---\n"
                status.update(
                    label="Upgrades/Downgrades available",
                    state="complete",
                    expanded=False,
                )
        ...
        with st.spinner("Generating Report"):
            ...
            report_message = (
                f"Please generate a report about: {ticker_to_research}\n\n\n"
            )
