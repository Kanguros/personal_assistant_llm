...
def main() -> None:
    ...
    search_type = st.sidebar.selectbox(
        "Select Search Type", options=["Knowledge Base", "Web Search (Tavily)"]
    )
            ...
            with st.status("Searching Knowledge", expanded=True) as status:
                with st.container():
                    ...
                    kb_search_docs: List[Document] = (
                        research_assistant.knowledge_base.search(
                            query=input_topic,
                    ...
                    if len(kb_search_docs) > 0:
                        ...
                        for idx, doc in enumerate(kb_search_docs):
                            ...
                            kb_search_results += (
                                f"{json.dumps(doc.meta_data, indent=4)}\n\n"
                            )
                ...
                status.update(
                    label="Knowledge Search Complete", state="complete", expanded=False
                )
