...
from pas.tools import Function, Tool, Toolkit
from pas.utils.format_str import remove_indent
from pas.utils.log import logger, set_log_level_to_debug
from pas.utils.merge_dict import merge_dictionaries
from pas.utils.message import get_text_from_message
...
class Assistant(BaseModel):
    ...
    run_id: str | None = Field(None, validate_default=True)
    # Run name
    ...
    additional_messages: list[dict | Message] | None = None
    ...
    add_to_system_prompt: str | None = None
    ...
    user_prompt: list | dict | str | None = None
    ...
    #     ...
    references_function: Callable[..., str | None] | None = None
    ...
    #     ...
    chat_history_function: Callable[..., str | None] | None = None
    ...
    def get_delegation_function(self, assistant: "Assistant", index: int) -> Function:
        ...
        assistant_name = (
            assistant.name.replace(" ", "_").lower()
            if assistant.name
            else f"assistant_{index}"
        )
    ...
    def update_llm(self) -> None:
        ...
        if (
            self.tool_call_limit is not None
            and self.tool_call_limit < self.llm.function_call_limit
        ):
    ...
    def from_database_row(self, row: AssistantRun):
        ...
        if row.llm is not None:
            ...
            if (
                llm_metrics_from_db is not None
                and isinstance(llm_metrics_from_db, dict)
                and self.llm
            ):
                try:
            ...
            try:
                if "chat_history" in row.memory:
                    self.memory.chat_history = [
                        Message(**m) for m in row.memory["chat_history"]
                    ]
                if "llm_messages" in row.memory:
                    self.memory.llm_messages = [
                        Message(**m) for m in row.memory["llm_messages"]
                    ]
                if "references" in row.memory:
                    self.memory.references = [
                        References(**r) for r in row.memory["references"]
                    ]
    ...
    def read_from_storage(self) -> AssistantRun | None:
    ...
    def write_to_storage(self) -> AssistantRun | None:
                ...
                if json_schema is not None:
                    ...
                    if json_schema_properties is not None:
                        for (
                            ...
                            output_model_properties[field_name] = (
                                formatted_field_properties
                            )
                    ...
                    if json_schema_defs is not None:
                        ...
                        for def_name, def_properties in json_schema_defs.items():
                            ...
                            if len(formatted_def_properties) > 0:
                                output_model_properties["$defs"][def_name] = (
                                    formatted_def_properties
                                )
    ...
    def get_formatted_chat_history(self) -> str | None:
    ...
    def _run(
        ...
        **kwargs: Any,
        ...
        # Otherwise, build the user prompt message
            ...
            # Get the user prompt
            user_prompt: list | dict | str | None = self.get_user_prompt(
                message=message,
                references=user_prompt_references,
                chat_history=user_prompt_chat_history,
            )
            # Create user prompt message
            user_prompt_message = (
                Message(role="user", content=user_prompt, **kwargs)
                if user_prompt
                else None
            )
        ...
        user_message = (
            Message(role="user", content=message) if message is not None else None
        )
    ...
    async def arun(
        ...
        *,
        ...
        messages: list[dict | Message] | None = None,
        **kwargs: Any,
    ) -> AsyncIterator[str] | str | BaseModel:
        # Convert response to structured output if output_model is set
        if self.output_model is not None and self.parse_output:
            logger.debug("Setting stream=False as output_model is set")
            resp = self._arun(
    ...
    def get_chat_history(self, num_chats: int | None = None) -> str:
        ...
        **kwargs: Any,
        ...
        if stream:
            ...
            _response = (
                Markdown(response)
                if self.markdown
                else self.convert_response_to_string(response)
            )
